...
def judge_command(args: argparse.Namespace, eval_results: Path) -> list[str]:
    command = [
        sys.executable,
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    try:
        ...
        if args.plan:
            subprocess.run(judge_command(args, eval_results), check=True)
            print(json.dumps({
                "upload": bool(bucket),
                "bucket": bucket,
                "aws_identity": identity,
                "writes": [
                    "v2-review/trajectory-runs/*",
                    "v2-review/trajectory-inbox/*",
                ] if bucket else [],

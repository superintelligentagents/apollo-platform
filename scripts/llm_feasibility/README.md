...
6. An independent quality check asks only whether the task is coherent and high quality and whether every step is aligned with the original request, in scope, and fair to evaluate. Each step must cite exact request text that authorizes it and identify any introduced requirement.
...
10. It writes the complete artifact to exactly one location under the selected
    `{review-root}` (`v2-review/` for `--queue v2`, `pc-review/` for `--queue pc`):
    - `{review-root}/llm_pass/{base64url(task_id)}.{task_content_hash}.{pipeline_version}.json` only when every worker says `POSSIBLE` and the manager says `FEASIBLE`.
    - `{review-root}/llm_fail/{base64url(task_id)}.{task_content_hash}.{pipeline_version}.json` for every other completed disposition, including worker errors.
...
The writable S3 prefixes are fixed to the selected queue family. The default is
`v2-review/llm_*`; `--queue pc` selects only `pc-review/llm_*`. CLI overrides
targeting submission, final-gold, the other app's queue, or arbitrary prefixes
are rejected before any model or AWS call.
...
- clear results upload only to `{review-root}/llm_pre_qc_pass/`;
- every other outcome uploads only to `{review-root}/llm_pre_qc_attention/`;
- claims use `{review-root}/llm_pre_qc_claims/`;
...
Apollo PC task sidecars are read from its separate reporting API and reviewed
into its separate prefix:

```bash
export APOLLO_REPORTING_URL='https://t1ynh195m1.execute-api.us-east-1.amazonaws.com/reporting/tasks'
...
- AWS CLI credentials authorized only for `s3:GetObject`/`s3:PutObject` on the
  selected queue's required `llm_*` prefixes. Do not grant one worker both
  `v2-review/*` and `pc-review/*` unless it is an explicitly supervised operator.
...
- Immutable S3 head and put operations use bounded retries and a 60-second per-attempt timeout, so a transient AWS error cannot silently strand a validated local artifact. A rerun reuses the validated artifact and republishes only the missing claim/result.
...
Pipeline v22 retains the legacy evergreen field as `NOT_ASSESSED` for API compatibility, but it does not invoke an evergreen model worker and the field does not affect pass/fail. Live, current, latest, price, schedule, and availability tasks are not rejected merely because their answers change.
...
- `manager_review.quality_review` is the legacy container name for the v22 combined task coherence/high-quality check and one alignment/fairness verdict per canonical step. Every step assessment records exact `request_support` and any `introduced_requirements`.

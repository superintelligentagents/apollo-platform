...
some SUBMIT SYNTHETIC DATA to the prod S3 bucket — always clean up afterwards
(delete `prolific/journeys/{test-participant}/` plus the matching
the matching `v2-review/` or `pc-review/` markers; every script uses throwaway participant
...
| `validate-author-appeal.mjs` | Complete V2 appeal lifecycle: anonymous first feedback, author rationale, different-reviewer routing, terminal second rejection under `rejected-twice/`, and no third chance | Requires `E2E_V2_REVIEW_KEY`; writes only disposable task/decision artifacts and deletes every exact key in `finally` |
| `e2e-review2.mjs` | v2 claim → navigate away → resume card → release; self-review exclusion | non-destructive |
| `validate-cross-app-review.mjs` | Production isolation check: each app-specific upload enters only its own completed-audit queue and remains excluded from its author | Set `E2E_V2_REVIEW_KEY` and `E2E_PC_REVIEW_KEY` (or `E2E_REVIEW_KEY` only when intentionally equal); deletes every exact disposable source, marker, lock, and audit object in `finally` cleanup |

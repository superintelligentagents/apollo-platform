...
if (!admin) throw new Error("E2E_ADMIN_EMAIL required");
if (!["v2", "pc"].includes(APP)) throw new Error("E2E_APP must be v2 or pc");
const REVIEW_KEY = process.env[APP === "pc" ? "E2E_PC_REVIEW_KEY" : "E2E_V2_REVIEW_KEY"] || process.env.E2E_REVIEW_KEY || "";
if (!REVIEW_KEY) throw new Error(`E2E_${APP.toUpperCase()}_REVIEW_KEY is required`);
...
const ENDPOINT = APP === "pc" ? (process.env.E2E_PC_REVIEW_ENDPOINT || "https://t1ynh195m1.execute-api.us-east-1.amazonaws.com") : (process.env.E2E_V2_REVIEW_ENDPOINT || "https://2fb2wkpayf.execute-api.us-east-1.amazonaws.com");
...
async function remove(key) {
  ...
  try { await s3.send(new HeadObjectCommand({Bucket:BUCKET,Key:key})); throw new Error('Object remains: '+key); }
...
const rawTaskId = `${APP}/${authorPid}/internal/${APP === "pc" ? "bundle" : "task"}-${stamp}`;
const safeTaskId = rawTaskId.replace(/[^A-Za-z0-9_-]/g, "_");
const sourceKey = `prolific/journeys/${authorPid}/${rawTaskId}/${Date.now()}-${randomUUID().slice(0, 8)}_${APP === "pc" ? "review_task_task-smoke.json" : "long_task.json"}`;
...
const finalGold = {
  ...
  reviewed_by: `Synthetic Reviewer ${stamp}`,
...
try {
  ...
  const showcase = await post('/review/admin', {admin_email:admin,action:'showcase'});
  check(showcase.examples.some(item=>item.task_id===rawTaskId), 'protected admin showcase includes the author-approved PC fixture');
  const detail = await post('/review/admin', {admin_email:admin,action:'detail',task_id:rawTaskId});
  check(detail.item?.status==='approved', 'admin detail loads the approved fixture');
  let filtered;
  for (let attempt=0;attempt<25;attempt++) {
    filtered = await post('/review/admin', {admin_email:admin,query:rawTaskId,participant_id:detail.item.participant_id});
    if(filtered.items.some(item=>item.task_id===rawTaskId)) break;
    await new Promise(resolve=>setTimeout(resolve,3000));
  }
  check(filtered.items.some(item=>item.task_id===rawTaskId), 'admin filtered list finds the fixture after cache refresh');
  ...
  const reopened = await post('/review/admin', {admin_email:admin,action:'reopen',task_id:rawTaskId,reason:'Disposable PC integration fixture'});
  check(reopened.ok && reopened.archived>=1, 'admin re-queue archives the fixture approval');
  const after = await post('/review/admin', {admin_email:admin,action:'detail',task_id:rawTaskId});
  check(after.item?.status==='pending', 'admin re-queue updates the dashboard status');
  const removedShowcase = await post('/review/admin', {admin_email:admin,action:'showcase'});
  check(!removedShowcase.examples.some(item=>item.task_id===rawTaskId), 're-queued task is removed from the author-approved showcase');
  const bulk = await post('/review/admin', {admin_email:admin,action:'reopen_by_reviewer',reviewer:finalGold.reviewed_by});
  check(bulk.matched===0 && bulk.reopened===0, 'bulk re-queue targets only the named synthetic reviewer');
} finally {
  const unit = sourceKey.slice(0,sourceKey.lastIndexOf('/')+1)+'review_task_task-smoke.json';
  for (const prefix of [`${ROOT}reopened/${b64url(unit)}/`,`${ROOT}reopen/${b64url(unit)}/`]) {
    const listed=await s3.send(new ListObjectsV2Command({Bucket:BUCKET,Prefix:prefix}));
    if (listed.IsTruncated) throw new Error('Unexpected fixture cleanup overflow');
    for(const entry of listed.Contents||[]) createdKeys.push(entry.Key);
  }

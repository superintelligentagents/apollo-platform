...
describe("PC author revision privacy", () => {
  const payload = { task_title: "Plan a visit", agent_request: "Use the supplied context.", difficulty: "high", success_criteria: [], steps: [], must_visit_or_reach: [], required_outputs: [], notes: null };
  it("blocks identifiers in edits and appeal reasons before upload", () => {
    const c = ctx();
    expect(() => assertAuthorEditPrivacy(c, { ...payload, agent_request: "Contact author@example.com for details." })).toThrow("Privacy check");
    expect(() => assertAuthorEditPrivacy(c, payload, "Please ask Synthetic Participant about this rejection.")).toThrow("Privacy check");
    expect(() => assertAuthorEditPrivacy(c, payload, "The provided source supports the requested task.")).not.toThrow();
  });
});


describe("PC upload identity parity", () => {
  it("queries tasks with the same protected participant id used for uploads", async () => {
    const c = ctx();
    c.state.identity!.participantId = "";
    mockMyTaskPage.mockResolvedValue({ items: [], offset: 0, limit: 200, source_total: 0, approved_total: 0, awaiting_signoff_total: 0 });
    renderMyTasks(c);
    await flush();
    const pid = mockMyTaskPage.mock.calls.at(-1)![1];
    expect(pid).toMatch(/^participant-/);
    expect(pid).not.toContain("author-example-com");
  });
});

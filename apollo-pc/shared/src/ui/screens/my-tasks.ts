...
import { auditPrivacyBundle } from "../../privacy-audit";
...
import { participantId as schemaParticipantId, participantUploadIdentity } from "../../schema";
...
function renderFeedback(
        ...
        "button",
        {
          ...
          onclick: async () => {
            ...
            try {
              await authorSignoff(
                ctx.state.reviewKey,
                schemaParticipantId(participantUploadIdentity(ctx.state.identity, ctx.state.entities)),
                item.sub_key,
                openedAt
              );
  ...
  const wireSubmit = (refs: EditFormRefs, editMode: Exclude<EditMode, null>) => {
    ...
    submitBtn.addEventListener("click", async () => {
      ...
      try {
        const pid = schemaParticipantId(participantUploadIdentity(ctx.state.identity, ctx.state.entities));
        const payload = refs.payload();
        assertAuthorEditPrivacy(ctx, payload, appealReason);
        if (editMode === "amend") {
          await authorAmend(ctx.state.reviewKey, pid, item.sub_key, payload, openedAt);
...
async function loadFeedback(
  ...
  try {
    const fb = await myTaskFeedback(ctx.state.reviewKey, schemaParticipantId(participantUploadIdentity(ctx.state.identity, ctx.state.entities)), item.sub_key);
...
export function renderMyTasks(ctx: Ctx): HTMLElement {
  ...
  if (!state.identity || !state.reviewKey) {
    body.append(state.identity ? renderReviewAccess(ctx) : el("p", { class: "muted" }, "Sign in to see your tasks."));
    root.append(backHome(ctx));
    return root;
  }

  const authorPid = schemaParticipantId(participantUploadIdentity(state.identity, state.entities));
...
// Author revisions bypass bundle serialization, so audit the exact outbound
// content (including appeal prose) before any mutation request is sent.
export function assertAuthorEditPrivacy(ctx: Ctx, edited: AuthorEditPayload, appealReason = ""): void {
  if (!ctx.state.identity) throw new Error("Sign in before submitting changes.");
  const audit = auditPrivacyBundle({
    files: [{ filename: "author_revision.json", body: JSON.stringify({ edited, appeal_reason: appealReason }) }],
    identity: ctx.state.identity,
    entities: ctx.state.entities,
    generatedAt: new Date().toISOString(),
  });
  if (audit.status === "blocked") {
    throw new Error("Privacy check: replace personal identifiers or credentials in your changes and appeal reason with aliases before submitting.");
  }
}

...
export function renderTrajectoryQueue(ctx: Ctx): HTMLElement {
  ...
  root.append(el("header", { class: "screen-head" }, el("p", { class: "eyebrow mono" }, "APOLLO PC · HUMAN TRAJECTORY QC"), el("h2", { class: "display" }, "Grade PC agent trajectories"), el("p", { class: "screen-sub" }, "This PC-only queue receives model-judged run packages from the pc-review trajectory folder. Independently grade the recorded browser path and every rubric.")));
  const resume = el("div");
  ...
  const refresh = async () => {
    ...
    try {
      ...
      body.replaceChildren(
        ...
        claimButton,
        el("p", { class: "muted small qc-queue-note" }, "The model judge's result is intentionally hidden during human grading. Your independent grade is saved in the PC trajectory queue, separately from the original task and run.")
      );
